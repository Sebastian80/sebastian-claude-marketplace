...
- topic="plugin.*" from source="bridge" - Bridge plugin lifecycle (hot-reload)
- topic="plugin.*" from source="claude" - Claude Code plugin events (installs/enables)
- topic="error" - Errors from any source
- topic="daemon.*" from source="bridge" - Daemon start/stop
"""
...
__all__ = ["Notifier", "NotifyLevel", "get_notifier", "init_notifier"]
...
class Notifier:
    ...
    def _format_deps_list(
        self, deps: list[str], prefix: str, max_show: int = 5
    ) -> list[str]:
        """Format dependency list for notification."""
        if not deps:
            return []
        lines = [""]
        for dep in deps[:max_show]:
            lines.append(f"  {prefix} {dep}")
        if len(deps) > max_show:
            lines.append(f"  ... +{len(deps) - max_show} more")
        return lines
    ...
    def handle_plugin_lifecycle(self, event: "Event") -> None:
        ...
        if event.topic == "plugin.loaded":
            lines = ["Plugin loaded"]
            lines.extend(self._format_deps_list(data.get("deps_installed", []), "+"))
            self.send(title, "\n".join(lines), NotifyLevel.SUCCESS)

        elif event.topic == "plugin.unloaded":
            lines = ["Plugin unloaded"]
            lines.extend(self._format_deps_list(data.get("deps_removed", []), "-"))
            self.send(title, "\n".join(lines), NotifyLevel.INFO)

        elif event.topic == "plugin.reloaded":
            lines = ["Plugin reloaded"]
            lines.extend(self._format_deps_list(data.get("deps_installed", []), "+", 3))
            lines.extend(self._format_deps_list(data.get("deps_removed", []), "-", 3))
            self.send(title, "\n".join(lines), NotifyLevel.INFO)
    ...
    def handle_claude_plugin(self, event: "Event") -> None:
        """Handle Claude Code plugin events.

        Topics: plugin.enabled, plugin.disabled, plugin.installed, plugin.uninstalled
        """
        data = event.data
        plugin_name = data.get("name", "Unknown")
        title = f"🔌 {plugin_name}"

        if event.topic == "plugin.enabled":
            lines = ["Plugin enabled"]
            lines.extend(self._format_deps_list(data.get("deps_installed", []), "+"))
            self.send(title, "\n".join(lines), NotifyLevel.SUCCESS)

        elif event.topic == "plugin.disabled":
            lines = ["Plugin disabled"]
            lines.extend(self._format_deps_list(data.get("deps_removed", []), "-"))
            self.send(title, "\n".join(lines), NotifyLevel.INFO)

        elif event.topic == "plugin.installed":
            version = data.get("version", "")
            msg = f"📥 Installed{' v' + version if version else ''}"
            self.send(title, msg, NotifyLevel.SUCCESS)

        elif event.topic == "plugin.uninstalled":
            version = data.get("version", "")
            msg = f"🗑️ Uninstalled{' v' + version if version else ''}"
            self.send(title, msg, NotifyLevel.INFO)
    ...
    def subscribe(self, bus: "EventBus") -> None:
        ...
        # Plugin lifecycle from bridge (hot-reload)
        bus.on(self.handle_plugin_lifecycle, source="bridge", topic="plugin.*")

        # Claude Code plugin events (installs/enables/disables)
        bus.on(self.handle_claude_plugin, source="claude", topic="plugin.*")
